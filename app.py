...
import psycopg2, os, qrcode, json, urllib.request
...
<div id="ai-panel" style="display:none;position:fixed;bottom:88px;right:24px;z-index:999;width:360px;max-width:calc(100vw - 32px);background:#fff;border-radius:16px;box-shadow:0 8px 40px rgba(0,0,0,0.18);overflow:hidden;flex-direction:column">
  ...
  <div id="ai-msgs" style="height:300px;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px;background:#F7F5F0">
    <div style="background:#fff;border-radius:4px 14px 14px 14px;padding:11px 13px;font-size:.83rem;color:#1C1C1C;max-width:90%;line-height:1.6;box-shadow:0 1px 4px rgba(0,0,0,0.06)">
      Hola! Puedo ayudarte con el sistema o con consultas contables e impositivas. Que necesitas?
      <div style="display:flex;flex-wrap:wrap;gap:5px;margin-top:10px">
        <button class="qbtn" onclick="sendQ('Como registro un pago')">Registrar pago</button>
        <button class="qbtn" onclick="sendQ('Como cierro la caja')">Cerrar caja</button>
        <button class="qbtn" onclick="sendQ('Vencimiento IVA AFIP')">Vto. IVA</button>
        <button class="qbtn" onclick="sendQ('Declaracion jurada AFIP')">DDJJ AFIP</button>
        <button class="qbtn" onclick="sendQ('Monotributo recategorizacion')">Monotributo</button>
...
.btn-google{background:#4285F4;color:#fff}.btn-afip{background:#0055a5;color:#fff}.btn-rentas{background:#1A3A2A;color:#fff}
</style>
<script>
var aiAbierto=false,aiHistorial=[];
var RESP={
  "pago":{t:"Para registrar un pago:\\n1. Anda a Clientes\\n2. Busca el cliente y toca Cuenta\\n3. En la fila del periodo toca Pagar\\n4. Ingresa monto, medio de pago y si hay factura ARCA\\n5. Confirma",l:[]},
  "caja":{t:"Para cerrar la caja:\\n1. Menu Caja\\n2. Revisa el resumen de cobros\\n3. Toca Cerrar Caja Hoy\\n4. Confirma\\n\\nQueda registrado con tu usuario y hora.",l:[]},
  "cliente":{t:"Para agregar un cliente:\\n1. Anda a Clientes\\n2. Completa nombre, CUIT, telefono, email y honorarios\\n3. Toca Guardar Cliente",l:[]},
  "recibo":{t:"Para generar un recibo PDF:\\n1. Anda a la cuenta del cliente\\n2. En el periodo toca Ver o PDF para descargarlo",l:[]},
  "agenda":{t:"La agenda de vencimientos esta en Menu -> Agenda.\\n\\nPodes ver los vencimientos del mes y actualizar el estado: Pendiente, Borrador, Presentado.",l:[]},
  "iva":{t:"Para presentar la DJ de IVA:",l:[{lb:"AFIP IVA",url:"https://www.afip.gob.ar/iva/",cls:"btn-afip"},{lb:"Buscar en Google",url:"https://www.google.com/search?q=como+presentar+DJ+IVA+AFIP+Argentina",cls:"btn-google"}]},
  "ddjj":{t:"Para declaraciones juradas en AFIP:",l:[{lb:"AFIP Declaraciones",url:"https://www.afip.gob.ar/declaraciones/",cls:"btn-afip"},{lb:"Buscar en Google",url:"https://www.google.com/search?q=como+presentar+declaracion+jurada+AFIP+Argentina",cls:"btn-google"}]},
  "mono":{t:"Para Monotributo y recategorizacion:",l:[{lb:"Monotributo AFIP",url:"https://www.afip.gob.ar/monotributo/",cls:"btn-afip"},{lb:"Buscar en Google",url:"https://www.google.com/search?q=monotributo+recategorizacion+AFIP",cls:"btn-google"}]},
  "iibb":{t:"Para Ingresos Brutos de Santiago del Estero:",l:[{lb:"Rentas Sgo. del Estero",url:"https://www.rentas.sde.gov.ar",cls:"btn-rentas"},{lb:"Buscar en Google",url:"https://www.google.com/search?q=ingresos+brutos+Santiago+del+Estero",cls:"btn-google"}]},
  "aportes":{t:"Para aportes y contribuciones F.931:",l:[{lb:"SUSS AFIP",url:"https://www.afip.gob.ar/suss/",cls:"btn-afip"},{lb:"Buscar en Google",url:"https://www.google.com/search?q=formulario+931+aportes+AFIP",cls:"btn-google"}]},
  "factura":{t:"Para facturas electronicas en ARCA:",l:[{lb:"Ir a ARCA",url:"https://www.afip.gob.ar/facturacion/",cls:"btn-afip"},{lb:"Como facturar",url:"https://www.google.com/search?q=como+emitir+factura+electronica+ARCA+AFIP",cls:"btn-google"}]},
  "venc":{t:"Para ver vencimientos impositivos:",l:[{lb:"Vencimientos AFIP",url:"https://www.afip.gob.ar/institucional/estudios/vencimientos.asp",cls:"btn-afip"},{lb:"Agenda del estudio",url:"/agenda",cls:"btn-rentas"},{lb:"Buscar en Google",url:"https://www.google.com/search?q=vencimientos+AFIP+calendario",cls:"btn-google"}]}
};
function cls(q){
  q=q.toLowerCase();
  if(/pago|cobr|registr/.test(q))return"pago";
  if(/caja|cerrar/.test(q))return"caja";
  if(/cliente|agreg/.test(q))return"cliente";
  if(/recibo|pdf/.test(q))return"recibo";
  if(/agenda/.test(q))return"agenda";
  if(/iva/.test(q))return"iva";
  if(/ddjj|declarac|jurada/.test(q))return"ddjj";
  if(/monotributo|monotrib/.test(q))return"mono";
  if(/ingresos brutos|iibb|brutos/.test(q))return"iibb";
  if(/aporte|contribuc|f931|suss/.test(q))return"aportes";
  if(/factur|arca/.test(q))return"factura";
  if(/vencimiento|vence/.test(q))return"venc";
  return null;
}
function addMsg(txt,tipo,links){
  ...
  if(links&&links.length){
    ...
    links.forEach(function(l){
      ...
      a.href=l.url;a.textContent=l.lb;
...
function typing(show){
  ...
  if(show&&!t){
    var box=document.getElementById('ai-msgs');
    var el=document.createElement('div');el.id='ai-typing';
    el.style.cssText='display:flex;align-items:center;gap:4px;padding:10px 13px;background:#fff;border-radius:4px 14px 14px 14px;width:52px;box-shadow:0 1px 4px rgba(0,0,0,0.06)';
    el.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:#C8A96E;animation:aib 1.2s infinite"></span><span style="width:6px;height:6px;border-radius:50%;background:#C8A96E;animation:aib 1.2s .2s infinite"></span><span style="width:6px;height:6px;border-radius:50%;background:#C8A96E;animation:aib 1.2s .4s infinite"></span>';
    box.appendChild(el);box.scrollTop=box.scrollHeight;
  } else if(!show&&t){t.remove();}
}
var sty=document.createElement('style');sty.textContent='@keyframes aib{0%,80%,100%{opacity:.2}40%{opacity:1}}';document.head.appendChild(sty);
function sendQ(q){document.getElementById('ai-input').value=q;sendAI();}
function sendAI(){
  ...
  var key=cls(q);
  if(key&&RESP[key]){
    setTimeout(function(){addMsg(RESP[key].t,'bot',RESP[key].l);},400);
    return;
  }
  ...
  .then(function(data){
    typing(false);btn.style.opacity='1';btn.disabled=false;
    if(data.respuesta){addMsg(data.respuesta,'bot',null);aiHistorial.push({role:'assistant',content:data.respuesta});}
    else{addMsg('No tengo esa respuesta. Te abro Google:','bot',[{lb:'Buscar en Google',url:'https://www.google.com/search?q='+encodeURIComponent(q+' Argentina contable'),cls:'btn-google'},{lb:'Ir a AFIP',url:'https://www.afip.gob.ar',cls:'btn-afip'}]);}
  })
  .catch(function(){
    typing(false);btn.style.opacity='1';btn.disabled=false;
    addMsg('Sin conexion. Intenta de nuevo.','bot',[{lb:'Buscar en Google',url:'https://www.google.com/search?q='+encodeURIComponent(q+' Argentina contable'),cls:'btn-google'}]);
  });
}
function toggleChat(){
  aiAbierto=!aiAbierto;
...
def nav_html(active=""):
    ...
    links_admin=[("/panel","Panel"),("/clientes","Clientes"),("/deudas","Deudores"),("/gastos","Gastos"),("/caja","Caja"),("/reportes","Reportes"),("/agenda","Agenda"),("/usuarios","Usuarios")]
...
def actualizar_db():
    conn=conectar();c=conn.cursor()
    for ddl in ["ALTER TABLE clientes ADD COLUMN IF NOT EXISTS email TEXT",
...
def panel():
    ...
    alertas=""
    if n_sin_abono>0:
        alertas+=f'<div class="warn-box"><b>{n_sin_abono}</b> clientes sin honorario asignado · <a href="/clientes" style="color:#7a5800;font-weight:600">Ver clientes</a></div>'
    if cajas_pendientes>0:
        alertas+=f'<div style="background:#fde8e8;border:1px solid #f5a0a0;border-radius:8px;padding:9px 14px;font-size:.82rem;color:#7a1a1a;margin-bottom:8px"><b>{cajas_pendientes}</b> secretaria(s) cobraron hoy y aun no cerraron su caja · <a href="/caja" style="color:#7a1a1a;font-weight:600">Ver caja</a></div>'
    ...
    <div class="fcard" style="margin-bottom:18px">
      <h3 style="border:none;padding:0;margin:0 0 12px">Ingresos vs Gastos - ultimos meses</h3>
      <div style="position:relative;height:220px"><canvas id="ch1"></canvas></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:18px" class="twocol">
      <div class="fcard" style="margin-bottom:0">
        <h3 style="border:none;padding:0;margin:0 0 10px">Cobros por socia</h3>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:18px" class="twocol">
      <div class="fcard" style="margin-bottom:0">
        <h3 style="border:none;padding:0;margin:0 0 12px">Gastos por categoria</h3>
        <div id="gcat" style="display:flex;flex-direction:column;gap:7px"></div>
      </div>
      <div class="fcard" style="margin-bottom:0">
        <h3 style="border:none;padding:0;margin:0 0 10px">Rendimiento acumulado</h3>
...
def clientes():
    ...
    for d in data:
        ...
        rows+=f'<tr data-search="{nombre.lower()} {(cuit or "").lower()} {(email or "").lower()}"><td class="nm">{nombre}</td><td class="mu">{cuit or "---"}</td><td class="mu">{tel or "---"}</td><td class="mu">{email or "---"}</td><td>{fmt(abono or 0)}</td><td style="white-space:nowrap;display:flex;gap:5px;flex-wrap:wrap"><a href="/cuenta/{cid}" class="btn btn-xs btn-p">Cuenta</a><a href="/editar_cliente/{cid}" class="btn btn-xs btn-o">Editar</a>{btn_del}</td></tr>'
    ...
    <div class="fcard"><h3>Nuevo Cliente</h3><form method="post">
      <div class="fgrid">
        <div class="fg"><label>Nombre / Razon Social</label><input name="nombre" required placeholder="Garcia Juan"></div>
        <div class="fg"><label>CUIT</label><input name="cuit" placeholder="20-12345678-9"></div>
        <div class="fg"><label>Telefono</label><input name="telefono" placeholder="3846000000"></div>
    ...
    <div class="dtable"><table>
      <thead><tr><th>Nombre</th><th>CUIT</th><th>Telefono</th><th>Email</th><th>Honorarios</th><th>Acciones</th></tr></thead>
...
def editar_cliente(id):
    ...
    if request.method=="POST":
        ...
        conn.commit()
        cambios=[f"{campo}: '{ant}' - '{nvo}'" for campo,ant,nvo in zip(["Nombre","CUIT","Telefono","Email","Honorarios"],antes,[nombre,cuit,tel,email,str(abono)]) if str(ant or "")!=str(nvo or "")]
        if cambios: registrar_auditoria("EDICION CLIENTE"," | ".join(cambios),id,nombre)
...
def cuenta(id):
    ...
    for h in historial:
        ...
        emitido=h[7] or h[1]
        hist_rows+=f'<div class="logrow"><div class="log-dot" style="background:var(--success)"></div><span class="log-time">{h[0]}</span><span class="log-user">{emitido}</span><span class="log-msg">Periodo <b>{h[2]}</b> - {fmt(h[3])} - <span class="bmedio">{h[4]}</span> - {fact_b}{" - <i>"+h[6]+"</i>" if h[6] else ""}</span></div>'
    ...
    <div class="mo" id="mp"><div class="modal">
      ...
      <form method="post">
        ...
        <div class="fg" style="margin-bottom:12px"><label>Observaciones</label><input name="observaciones" placeholder="Ej: cheque N 12345..."></div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;padding:10px;background:#f0f9f4;border-radius:8px">
          ...
          <label for="chk-fact" style="font-size:.84rem;font-weight:600;color:var(--primary);cursor:pointer">Se emitio factura en ARCA para este pago</label>
        </div>
        <div class="mact">
...
def deudas():
    ...
    body=f'<h1 class="page-title">Deudores</h1><p class="page-sub">{len(data)} clientes - Total: <b>{fmt(total)}</b></p>{cards or "<p style=color:var(--muted);text-align:center;padding:48px>Sin deudores</p>"}'
...
def gastos():
    ...
    res_html="";mx_g=resumen[0][1] if resumen else 1
    ...
    es_admin=session.get("rol")=="admin";rows=""
    for g in lista:
    ...
    <p class="page-sub">Control de egresos</p>{flash}
    <div class="stats">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px" class="twocol">
      <div class="fcard" style="margin-bottom:0"><h3>Registrar Gasto</h3>
        <form method="post"><input type="hidden" name="accion" value="crear">
          <div class="fgrid">
            <div class="fg"><label>Categoria</label><select name="categoria">{cat_opts}</select></div>
            <div class="fg"><label>Monto $</label><input name="monto" type="number" step="0.01" required placeholder="0"></div>
            <div class="fg"><label>Descripcion</label><input name="descripcion" placeholder="Detalle opcional"></div>
    ...
    <div class="dtable"><table>
      <thead><tr><th>Fecha</th><th>Categoria</th><th>Descripcion</th><th>Monto</th><th>Cargado por</th><th></th></tr></thead>
...
def reportes():
    ...
    body=f"""
    <h1 class="page-title">Reportes</h1><p class="page-sub">Resumenes financieros y auditoria</p>{sin_ab}
    <div class="fcard"><h3>Distribucion entre Socias</h3>
    ...
    <div class="tabs">
      ...
      <button class="tab" onclick="showTab('t5',this)">Auditoria</button>
    </div>
    ...
    <div id="t4" class="tabpanel"><div class="dtable"><table><thead><tr><th>Categoria</th><th>Total</th></tr></thead><tbody>{filas_gastos or "<tr><td colspan=2 style=color:var(--muted);text-align:center;padding:20px>Sin gastos</td></tr>"}</tbody></table></div></div>
...
def usuarios():
    ...
    if request.method=="POST":
        ...
        if accion=="crear":
            ...
            if not usuario or not clave: flash='<div class="flash ferr">Completa usuario y contrasena</div>'
            else:
                ...
                if c.fetchone(): flash='<div class="flash ferr">Ese usuario ya existe</div>'
                    ...
                    flash=f'<div class="flash fok">Usuario {display} creado</div>'
            ...
            if nueva and len(nueva)>=4:
                ...
                conn.commit();flash='<div class="flash fok">Contrasena actualizada</div>'
            else: flash='<div class="flash ferr">Minimo 4 caracteres</div>'
            ...
            conn.commit();flash='<div class="flash fok">Datos actualizados</div>'
            ...
            else: flash='<div class="flash ferr">No podes eliminar tu propio usuario</div>'
    ...
    body=f"""
    <h1 class="page-title">Gestion de Usuarios</h1><p class="page-sub">Usuarios, contrasenas y permisos</p>{flash}
    <div class="fcard" style="border-left:4px solid var(--accent)"><h3>Mis Datos - {session.get("display","")}</h3>
      <form method="post"><input type="hidden" name="accion" value="cambiar_mis_datos">
        <div class="fgrid">
          ...
          <div class="fg"><label>Nueva contrasena (vacio = no cambia)</label><input name="nueva_clave_admin" type="password" placeholder="Nueva contrasena"></div>
        </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px" class="twocol">
      <div class="fcard" style="margin-bottom:0"><h3>Nuevo Usuario</h3>
        <form method="post"><input type="hidden" name="accion" value="crear">
          <div class="fgrid" style="grid-template-columns:1fr">
            <div class="fg"><label>Nombre completo</label><input name="nombre_display" placeholder="Maria Gonzalez"></div>
            <div class="fg"><label>Usuario</label><input name="usuario" placeholder="mariag" required></div>
            <div class="fg"><label>Contrasena</label><input name="clave" type="password" placeholder="Minimo 4 caracteres" required></div>
      ...
      <div class="fcard" style="margin-bottom:0"><h3>Permisos</h3>
        <div class="info-box" style="margin-bottom:10px"><b>Admin:</b> Panel, graficos, reportes, todos los modulos</div>
        <div style="background:#f0f4ff;border:1px solid #b0c4ee;border-radius:8px;padding:10px 14px;font-size:.8rem;color:#1a3a8a"><b>Secretaria:</b> Clientes, pagos, recibos, gastos, caja, agenda, deudores. Sin acceso a panel financiero, reportes ni usuarios.</div>
      </div>
    </div>
    <div class="fcard"><h3>Usuarios ({len(lista)})</h3>{cards or '<p style="color:var(--muted);font-size:.84rem">Sin usuarios</p>'}</div>
    <div class="mo" id="mc"><div class="modal"><h3>Cambiar Contrasena</h3><p class="msub" id="mc-sub"></p>
      <form method="post"><input type="hidden" name="accion" value="cambiar_clave"><input type="hidden" name="uid" id="mc-uid">
        <div class="fg" style="margin-bottom:14px"><label>Nueva contrasena</label><input name="nueva_clave" type="password" placeholder="Minimo 4 caracteres" required></div>
